...
// reload_agents does nothing itself beyond returning a fixed result — it
// exists purely as a signal a running turn can emit to ask for the whole
// agent tree (tools, sub-agents, MCP servers) to be rebuilt from
// whatever's currently on disk, same as picking /reload-agents by hand.
// Destructive in effect (it changes what the agent itself is capable
// of), so it requires confirmation in "normal" mode same as write_file —
// an agent only gets to skip that in full-auto, which is an explicit,
// already-accepted trust decision, not something this tool needs its own
// separate safeguard for. It touches no filesystem path, so resources is
// nil (no conflict gating).
func init() {
	register(spec{
		destructive: true,
		resources:   nil,
