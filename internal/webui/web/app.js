...
window.summarizeArgs  = (name, args) => formatToolArgs(name, args);
window.summarizeResult = (name, result, args) =>
  formatToolResult(name, args, result, A().verboseTools, effectiveToolPreviewLines());

// ══════════════════════════════════════════════════════════════════
//  Tool output formatting — port of internal/ui/toolformat.go. Lean and
//  verbose deliberately show categorically different things (a bare
//  count/size vs the actual content), not the same JSON at two lengths;
//  keep any change here in lockstep with the Go file.
// ══════════════════════════════════════════════════════════════════
const ARG_PREVIEW_MAX = 60;             // maxArgValuePreview
const TOOL_PREVIEW_LINES_DEFAULT = 25;  // toolPreviewMaxLinesDefault

function effectiveToolPreviewLines() {
...
function formatToolResult(name, args, result, verbose, maxLines) {
